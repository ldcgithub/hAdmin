$(function(){
    ...
    $(".J_menuItem").on('click',function(){
        var url = $(this).attr('href');
        var name = $(this).text();
        if(name=='借款初审'){name='发标管理'}
        if(name=='满标待审'){name='满标复审'}
        if(name=='借款授权待审'){name='用户列表'}
        if(name=='今日待还'){name='还款管理'}
        if(typeof(url) != "undefined"){
            //$("#J_iframe").attr('src',url);
            $('.page-tab-ul li',window.parent.document).removeClass('bg_color');


            /*
             * 生成TAB，先判断要请求的页面是否已存在，存在直接显示，不存在新建新的tab
             * */
            var iframetabrel = $('li[url="'+url+'"]',window.parent.document).attr('rel');
            if(typeof(iframetabrel) != 'undefined'){
                //存在，直接显示
                parent.show_iframe(iframetabrel);
                //$(window.parent.document).find('iframe[page-rel="'+iframetabrel+'"]').attr('src',url); // 刷新点击页面
            }else{
                //不存在，新建iframe
                if(url != '/Center/welcome'){
                    var maxrelnum = $('#page-tab ul li:last',window.parent.document).attr('rel');//    最大rel
                    var newrel = Number(maxrelnum) + 1;
                    $('#page-tab ul',window.parent.document).append('<li class="bg_color" rel="'+newrel+'" url="'+url+'"><span onclick="parent.show_iframe('+newrel+')">'+name+'</span><span class="c-icom" onclick="close_iframe('+newrel+');" onmouseover="parent.mouseoverBtn()" onmouseout="parent.mouseoutBtn()" ><img id="close-icon" src="../Public/hAdmin/img/close.png"></span></li>');
                    $('#page-list iframe',window.parent.document).css('display','none');
                    $('#page-list',window.parent.document).append('<iframe page-rel="'+newrel+'" width="100%" height="100%" src="'+url+'" frameborder="0" seamless></iframe>');
                    iframetabrel = newrel;
                }else{
                    parent.show_iframe(0);
                    iframetabrel = 0;
                }
            }
            //  计算左边li长度总和
            var leftLegth = parent.calUlLength(iframetabrel);
            //  当前向左偏离长度
            var leftAbsolut = Math.abs(parseInt($('.page-tab-ul',window.parent.document).css('left')));
            //  可视区长度1003.19
            var visiableLength = $('.page_content',window.parent.document).width() - 35 - 101;
            ...
            if(Number(visiableLength) + Math.abs(Number(leftAbsolut))<Number(leftLegth)){
                ...
                parent.click_left(leftAdd + leftAbsolut)
            }
...
/*function show_iframe(rel){
    $('#page-list iframe',window.parent.document).css('display','none');
    $('iframe[page-rel="'+rel+'"]',window.parent.document).css('display','block');

    //  新加
    $('.page-tab-ul li',window.parent.document).removeClass('bg_color');
    $('li[rel="'+rel+'"]',window.parent.document).addClass('bg_color');
...
function calUlLength(rel){
    var length = 0;
    $('.page-tab-ul li[rel!="'+rel+'"]',window.parent.document).each(function(){
